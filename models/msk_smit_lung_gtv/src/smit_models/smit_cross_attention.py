...
class SwinTransformer_wDualModalityFeatureTalk_OutSum_5stageOuts(nn.Module):
    ...
    def forward(self, x):
        ...
        #print (' info: after patch merge layer size ',out.view(-1, Wh_x0, Ww_x0, Wt_x0, self.embed_dim).permute(0, 4, 1, 2, 3).shape)
        x_0 = self.patch_merging_layers[0](x_0, Wh_x0, Ww_x0, Wt_x0)
        x_1 = self.patch_merging_layers[0](x_1, Wh_x1, Ww_x1, Wt_x1)
        ...
        #print (' info: after Stage 0 layer size ',out.shape)
        outs.append(out)  # layer 0 output
        # add transformer encoded back to modality-specific branch
        x_0 = x_0_out + x_0
        x_1 = x_1_out + x_1
        ...
        norm_layer = getattr(self, f'norm{1}')
        x_out_l1 = norm_layer(out_x0_x1_l1)
        out = x_out_l1.view(-1, int(H_x0_1), W_x0_1, T_x0_1, self.embed_dim * 4).permute(0, 4, 1, 2,3).contiguous()
        #print (' info: after Stage 1 layer size ',out.shape)
        outs.append(out)  # layer 1 output
        # add transformer encoded back to modality-specific branch
        x_0 = x_0_out + x_0
        x_1 = x_1_out + x_1
        ...
        norm_layer = getattr(self, f'norm{2}')
        x_out_l2 = norm_layer(out_x0_x1_l2)
        out = x_out_l2.view(-1, int(H_x0_1), W_x0_1, T_x0_1, self.embed_dim * 8).permute(0, 4, 1, 2, 3).contiguous()

        #print (' info: after Stage 2 layer size ',out.shape)
        outs.append(out)  # layer 2 output
        # add transformer encoded back to modality-specific branch
        x_0 = x_0_out + x_0
        x_1 = x_1_out + x_1
        ...
        norm_layer = getattr(self, f'norm{3}')
        x_out_l3 = norm_layer(out_x0_x1_l3)
        out = x_out_l3.view(-1, int(H_x0_1), W_x0_1, T_x0_1, self.embed_dim * 16).permute(0, 4, 1, 2,3).contiguous()
        
        
        #print (' info: after Stage 3 layer size ',out.shape)                                                                                                                                                                
        outs.append(out)  # layer 2 output
...
class SMIT_Cross_Attention(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int = 2,
    ...
    def forward(self, x_in):
        ...
        print ('encoder enc11 size is ',enc11.size())
        print ('encoder enc22 size is ',enc22.size())
        print ('encoder enc33 size is ',enc33.size())
        print ('encoder enc44 size is ',enc44.size())

        #encoder x after projection size is  torch.Size([1, 768, 4, 4, 4])
        #encoder enc11 size is  torch.Size([1, 48, 64, 64, 64])
        #encoder enc22 size is  torch.Size([1, 96, 32, 32, 32])
        #encoder enc33 size is  torch.Size([1, 192, 16, 16, 16])
        #encoder enc44 size is  torch.Size([1, 384, 8, 8, 8])
        ...
        x_prim = torch.unsqueeze(x_in[:, 0, :, :, :], 1)
        x_second = torch.unsqueeze(x_in[:, 1, :, :, :], 1)

        print ('x_prim size ',x_prim.shape)
...
class SMIT_Cross_Attention_24(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int = 2,
